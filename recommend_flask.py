...
books = pd.read_csv('/home/lucas/flaskit/recommendations_model/Books/BX-Books.csv', sep=';', error_bad_lines=False, encoding="latin-1")
books.columns = ['ISBN', 'bookTitle', 'bookAuthor', 'yearOfPublication', 'publisher', 'imageUrlS', 'imageUrlM', 'imageUrlL']
users = pd.read_csv('/home/lucas/flaskit/recommendations_model/Books/BX-Users.csv', sep=';', error_bad_lines=False, encoding="latin-1")
users.columns = ['userID', 'Location', 'Age']
ratings = pd.read_csv('/home/lucas/flaskit/recommendations_model/Books/BX-Book-Ratings.csv', sep=';', error_bad_lines=False, encoding="latin-1")
...
# Ahora que estan bien los anos, se pasa a valores numericos
books.yearOfPublication=pd.to_numeric(books.yearOfPublication, errors='coerce')
# Se deja en NULL los anos 0 o mayores a 2006
...
#Se calzan las tablas para que tengan los mismos id de usuarios e items
...
######## Training ########
# Divide el DataFrame en train y test trasnformandolos en una matriz de esparcion.
# Tambien devuelve raw_train_df que es train en forma de dataframe.
def informed_train_test(rating_df, train_ratio):
    ...
    # Deja a los usuarios y libros que esten en train y en test
    ...
    for k in id_cols:
        ...
        # Transforma todas las columnas a categorias numericas
    ...
    cate_enc = preprocessing.LabelEncoder()
    ratings = dict()
    # Transforma la columna bookRating a categorica numerica
    ...
    # ubicacion correspondiente de (row, col)
    ...
    test = coo_matrix((ratings['test'], (trans_cat_test['userID'], \
                                                        trans_cat_test['ISBN'])) \
                                     , shape=(n_users, n_items))
    # Retorna una matriz de esparcion, como la matriz de interaccion, pero con los parametros entregados.
...
# Se calcula el area bajo la curva (AUC)
start_time = time.time()
# Se crea el modelo con la funcion loss='Weighted Approximated-Rank Pairwise (warp)'
...
# Aqui se le entrega una matriz de esparcion (la mayoria de sus elementos son 0) y se entrena el modelo.
...
# Diccionario de usuarios e items
...
def similar_recommendation(user_id,model=model, interaction_matrix=user_item_matrix, user_dikt=user_dikt,item_dikt=item_dikt,threshold = 0,number_rec_items = 15):
    ...
    scores.index = interaction_matrix.columns #Se obtiene el score dela matriz de interaccion y se ordena
    ...
    # Se obtienen los items con los que ya interactuo el usuario y valoro por sobre el threshold
    ...
    return jsonify({'Recomendaciones':scores})

@app.route('/item/<ISBN>',methods=['GET'])
# Recomendacion de un item para una lista de usuarios
def users_for_item(ISBN,model=model,interaction_matrix=user_item_matrix,number_of_user=15):

    n_users, n_items = interaction_matrix.shape
    # x es una lista de los book_id
    x = np.array(interaction_matrix.columns)
    # A .predict se le pasan todos los usuarios y se calcula la afinidad con un item en especifico
                                    # searchsorted busca en la lista x la posicion de ISBN
